...
def preprocess_train_data(train_df):
    ...
    for row in train_df.index.values:
      if(len(train_df.at[row,"text"].split()) > 50):
        train_df = train_df.drop(labels=row, axis=0)
      else:
        train_df.at[row,"text"] = deEmojify(train_df.at[row,"text"])
...
if __name__ == "__main__":
    ...
    model = word2vec.Word2Vec(sentences=train_df["tokens"].tolist(),vector_size = 300, sg=1,workers=4, min_count=2)
